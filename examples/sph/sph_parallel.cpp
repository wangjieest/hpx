    ...
    float mass)
{
    ...
    for_loop(par, 0, rho.size(),
        [&mtx, &rho, &pos_x, &pos_y, h_squared, C](std::size_t i)
        {
            for (std::size_t j = i + 1; j < pos_x.size(); ++j)
            {
    ...
    float mass, float g, float h, float k, float rho0, float mu)
{
    ...
    for_loop(par, 0, pos_x.size(),
        [&mtx, &pos_x, &pos_y, &v_x, &v_y, &a_x, &a_y, &rho, h_squared, h, rho0, C_0, C_p, C_v]
        (std::size_t i)
        {
            for (std::size_t j = i + 1; j < pos_x.size(); ++j)
            {
    ...
    float dt)
{
    ...
    for_loop(par, 0, pos_x.size(), [dt, &pos_x, &pos_y, &v_x, &v_y, &a_x, &a_y]
        (std::size_t i)
        {
            v_x[i] += a_x[i] * dt;
            v_y[i] += a_y[i] * dt;
    ...
    std::vector<float>& v_y)
{
    ...
    for_each(par, begin, end,
        [XMIN, XMAX, YMIN, YMAX](decltype(*begin) it)
        {
